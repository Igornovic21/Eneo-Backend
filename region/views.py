    ...
    def list(self, request):
        ...
        for region in regions:
            ...
            if min_date is not None:
                ...
                records = records.only("date").filter(date__gte=make_aware(date, timezone=pytz.UTC)).only("itinary").filter(itinary__region=region)
            if max_date is not None:
                date = datetime.strptime(max_date, DATETIME_FORMAT)
                records = records.only("date").filter(date__lte=make_aware(date, timezone=pytz.UTC)).only("itinary").filter(itinary__region=region)
